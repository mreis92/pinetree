...
PRIVATE double  get_energy(pu_contrib *p_c, int align_length, int end_pos);
...
/*--------------------------------------------------------------------------*/
double RNAup(int argc, char *argv[], char* input_string, int region, int end_pos){
  ...
  char                    *head, *s1, *s2, *s3, *s_target, *cstruc1, *cstruc2, *cstruc_target, *cstruc_combined;
  ...
  double                  energy, min_en, best_energy;
  double                  sfact=1.07;
  int                     noconv=0;
  int                     max_u = 0;
  ...
  do_backtrack    = 1;
  rotated         = 0;
  s1 = s2 = s3 = s_target = cstruc1 = cstruc2 = cstruc_target = cstruc_combined = NULL;
  ...
  */
	...
	*/
	...
	/*if(read_again){
		printf("Reading second sequence\n");
	...
	*/
	...
	switch(up_mode){
		case RNA_UP_MODE_1:   
							best_energy = get_energy(unstr_out, region, end_pos);
...
PRIVATE void print_interaction(interact *Int, char *s1, char *s2, pu_contrib *p_c, pu_contrib *p_c2, int w, int incr3, int incr5) {
  ...
  printf("Total free energy of binding: %f kcal/mol\n", G_min);
  printf("Energy from duplex formation: %f kcal/mol\n", Gi_min);
  printf("Opening energy for the longer sequence: %f kcal/mol\n", Gul);
  printf("Opening energy for the shorter sequence: %f kcal/mol\n", Gus);
...
PRIVATE void print_unstru(pu_contrib *p_c, int w) {
  ...
  if (p_c != NULL) {
    ...
    printf("Best accessibility (%4d,%4d) = (%.3f)\n", min_i,min_j,min_gu);
...
PRIVATE double get_energy(pu_contrib *p_c, int align_length, int end_pos) {
  int i,j,len,min_i,min_j;
  double dG_u, min_gu, blubb;

  i = end_pos-align_length+1;
  j = end_pos;
  blubb = p_c->H[i][j-i]+p_c->I[i][j-i]+p_c->M[i][j-i]+p_c->E[i][j-i];
  dG_u = -RT*log(blubb);
  return dG_u;
